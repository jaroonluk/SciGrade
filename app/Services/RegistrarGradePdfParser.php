...
class RegistrarGradePdfParser
{
    ...
    /**
     * อ่านคณะของนักศึกษาจากหัวข้อกลุ่มในเนื้อหา
     * - บรรทัดขึ้นต้นด้วย "คณะ..."
     * - หรือชื่อหน่วยงานตรง ๆ เช่น "วิทยาลัยการคอมพิวเตอร์" / "สถาบันภาษา" (ไม่มีคำว่าคณะนำหน้า)
     * ไม่ใช้บรรทัดหัวกระดาษเช่น "ปริญญาตรี ภาคปกติ คณะวิทยาศาสตร์"
     *
     * @return list<string>
     */
    private function parseFaculties(string $text): array
    {
        ...
        foreach (explode("\n", $text) as $line) {
            ...
            if (preg_match('/^คณะ(.+)$/u', $line, $match)) {
                ...
                // หัวข้อคณะแบบไม่มีคำว่า "คณะ" นำหน้า
                $name = $line;
            }

            // ข้ามบรรทัดที่ปนข้อมูลนักศึกษา / ไม่ใช่หัวข้อคณะ
            if ($name === '' || preg_match('/(?:นาย|นาง|นางสาว|<>|\d{9}|วิทยาเขต)/u', $name)) {
                continue;
            }
