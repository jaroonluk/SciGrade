    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-[#7A4A3A]/80 mt-1">
                แต่ละสาขากำหนดรหัสวิชา (เงื่อนไข) ที่ใช้กรองรายงาน — ไม่แยกตามระดับการศึกษา
            </p>
        </div>
    ...
    <div class="space-y-4">
        @forelse ($departments as $dept)
            @php
                ...
                $patterns = $dept->patterns ?? collect();
                $details = $dept->pattern_details ?? [];
            @endphp
                ...
                <div class="px-4 py-3 bg-gradient-to-r from-[#FFFBF7] to-[#FAF0E6]/60 border-b border-[#E8C4B8]/70 flex flex-wrap items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-[#7A4A3A]/75 mt-0.5">
                            ...
                            · มีรหัสเงื่อนไข <strong>{{ $dept->pattern_count ?? $patterns->count() }}</strong> รายการ
                        </p>
                    </div>
                    <form method="POST" action="{{ route('faculty-admin.department-patterns.restore') }}"
                          onsubmit="return confirm('กู้คืนค่าเริ่มต้นของสาขา {{ $dept->department_name }}?\nเงื่อนไขปัจจุบันจะถูกแทนที่ด้วยค่าเริ่มต้น')">
                        @csrf
                        <input type="hidden" name="department_id" value="{{ $dept->department_id }}">
                        <input type="hidden" name="q" value="{{ $q }}">
                        <button type="submit" class="px-2.5 py-1 border border-amber-300 text-[#5C2E1F] rounded-lg text-[0.7rem] hover:bg-white">
                            กู้คืนค่าเริ่มต้น
                        </button>
                    </form>
                </div>

                <div class="px-3 py-2.5 border-b border-amber-100 bg-amber-50/40">
                    <div class="flex flex-wrap gap-2">
                        ...
                        @empty
                            <p class="text-xs text-amber-800">ยังไม่มีเงื่อนไขรหัสวิชา — สาขานี้จะไม่พบรายวิชาจนกว่าจะเพิ่มรหัส</p>
                        @endforelse
                    </div>
                </div>
                        ...
                        <div class="pattern-row" data-pattern-row>
                            <div class="pattern-view flex flex-wrap items-center gap-2 w-full justify-between">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <button type="button" class="btn-edit-pattern px-2.5 py-1 border border-amber-300 rounded text-xs hover:bg-white">แก้ไข</button>
                            ...
                            <form method="POST" action="{{ route('faculty-admin.department-patterns.update', $row->id) }}" class="pattern-edit-form flex-wrap items-center gap-2">
                                ...
                                <button type="submit" class="px-3 py-1.5 text-white rounded text-xs bg-[#8B4513] hover:bg-[#6B3410]">บันทึก</button>
                                <button type="button" class="btn-cancel-edit px-3 py-1.5 border border-amber-300 rounded text-xs hover:bg-white">ยกเลิก</button>
                            </form>
                        </div>
                    @endforeach
                </div>

                <div class="px-3 py-3 bg-[#FAF0E6]/35">
                    <form method="POST" action="{{ route('faculty-admin.department-patterns.store') }}" class="flex flex-wrap items-end gap-2">
                        @csrf
                        <input type="hidden" name="department_id" value="{{ $dept->department_id }}">
                        <input type="hidden" name="q" value="{{ $q }}">
                        <div class="flex-1 min-w-[12rem]">
                            <label class="block text-xs font-medium mb-1 text-[#5C2E1F]">เพิ่มรหัสเงื่อนไข</label>
                        ...
                        <button type="submit" class="px-4 py-2 text-white rounded-lg text-sm font-medium bg-green-700 hover:bg-green-800">
                            เพิ่ม
                        </button>
                    </form>
                </div>
